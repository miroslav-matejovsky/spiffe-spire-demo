...
Write-Host "Provisioning demo x509pop agent materials..." -ForegroundColor Cyan

$caCertPath = Join-Path $serverDir "agent-cacert.pem"
$agentKeyPath = Join-Path $agentDir "agent.key.pem"
$agentCertPath = Join-Path $agentDir "agent.crt.pem"

$generatedFiles = @($caCertPath, $agentKeyPath, $agentCertPath)
