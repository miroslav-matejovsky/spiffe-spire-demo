...
$agentDir = Join-Path $scenarioRoot "spire\agent-2"

New-Item -ItemType Directory -Path $serverDir -Force | Out-Null
New-Item -ItemType Directory -Path $agentDir -Force | Out-Null

Write-Host "Provisioning demo DevID materials for agent-2..." -ForegroundColor Cyan

$caCertPath = Join-Path $serverDir "devid-ca.pem"
$caKeyPath = Join-Path $serverDir "devid-ca-key.pem"
$agentCaPath = Join-Path $agentDir "devid-ca.pem"
...
Set-Content -Path $caCertPath -Value $caPem -NoNewline
Set-Content -Path $caKeyPath -Value $caKey.ExportPkcs8PrivateKeyPem() -NoNewline
Set-Content -Path $agentCaPath -Value $caPem -NoNewline
